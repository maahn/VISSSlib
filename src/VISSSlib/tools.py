...
def detectCaptureIdDropTimes(
    ...
    drop, but `estimateCaptureIdDiffCore` averages the whole window
    into one offset and fails its >70% consistency check whenever a
    drop happens to fall inside it. The offset itself is not
    ambiguous -- only the single-offset-per-window assumption is
    ...
    leaderDat, followerDat : xarray.Dataset
        Same inputs as `estimateCaptureIdDiffCore`.
    dim : str, optional
